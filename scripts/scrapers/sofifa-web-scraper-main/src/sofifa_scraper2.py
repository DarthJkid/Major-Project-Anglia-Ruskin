...
class SoFIFAScraper:
    def __init__(
        ...
        # Locks for safe parallel writing / ID updates
    ...
    def load_existing_player_ids(self):
        """Load player IDs already present in the output CSV"""
    ...
    def _get_column_order(self):
        """Fixed, stable column order for the CSV"""
        return [
    ...
    def _normalize_row_for_csv(self, stats):
        """Make sure each row has the exact same columns"""
    ...
    async def save_player_to_csv(self, stats):
        """Save one player's stats safely while scraping in parallel"""
    ...
    async def build_context(self, browser):
        """
        Build one persistent browser context.
        This is the main change vs your old version:
        one context is reused across many players instead of creating
        a fresh context for every player.
        """
    ...
    async def prepare_page(self, context):
        """
        Create a page inside the persistent context and apply stealth/resource rules.
        """
    ...
    async def scrape_one_player(self, page, url, idx, total):
        """
        Scrape one player using an already-open page inside a persistent context.
        """
        ...
        while retries < max_retries:
            try:
                # Human-like pause before request
    ...
    async def scrape_batch(self, browser, batch_urls, batch_start_index, total):
        """
        Scrape one batch using a single persistent context and a small page pool.
        """
        context = await self.build_context(browser)

        try:
            # Create a small reusable page pool
            page_pool = []
            ...
            async def worker(worker_id, page):
                ...
                while not queue.empty():
                    ...
                    # Small pause after each player
                    await asyncio.sleep(random.uniform(0.8, 1.8))

                    # Periodic worker cooldown
                    if processed > 0 and processed % 10 == 0:
                        cooldown = random.uniform(2, 3)
    ...
    async def scrape_player_stats(self, max_players=None):
        """
        Scrape player stats in batches.
        Each batch gets a fresh context, but players within the batch reuse it.
        This is much more stable than one context per player, while still allowing
        periodic session refreshes for long runs.
        """
        ...
        async with async_playwright() as p:
            ...
            for batch_num, batch_start in enumerate(range(0, total, self.batch_size), start=1):
                ...
                if batch_start + self.batch_size < total:
                    batch_pause = random.uniform(10, 15)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of pages to scrape in parallel inside one context"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=1000,
        help="Number of players per context refresh batch"
    )
